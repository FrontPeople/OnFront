...
async function analyzeImageWithAI(base64DataUrl, sourceName = "Foto Kemasan") {
  ...
  try {
    ...
    // -----------------------------------------------------------------------
    // STEP 1: OCR pass - extract raw text from the packaging photo
    // -----------------------------------------------------------------------
    const ocrPrompt = `You are an expert OCR engine specialized in reading food packaging labels.
Your PRIMARY task is to read the provided food packaging image and extract the EXACT, VERBATIM text from the KOMPOSISI / INGREDIENTS / COMPOSITION / BAHAN-BAHAN section, including ALL sub-ingredients, E-numbers, and additive codes. Do NOT paraphrase, summarize, or translate any ingredient - reproduce it EXACTLY as it appears on the packaging, character by character.

Also extract:
- The EXACT product name as printed on the packaging.
- The EXACT brand/manufacturer name as printed.
- Country of origin or "Dibuat di" / "Made in" text.
- Any registration number (BPOM, P-IRT, etc.).
- Any expiry / best before date.
- Any Halal certification logo or text present.

Return STRICT JSON ONLY (no markdown, no explanation outside JSON):
{
  "productName": "Exact product name from packaging",
  "brandName": "Exact brand/manufacturer from packaging",
  "countryOfOrigin": "Exact country text from packaging",
  ...
  "rawIngredientsText": "VERBATIM full KOMPOSISI/INGREDIENTS section text copied letter-by-letter from the packaging",
  ...
  "ocrNotes": "Brief note if any text was unclear or partially unreadable"
    ...
    if (loadingStepText) loadingStepText.textContent = "📖 Membaca teks kemasan secara verbatim (OCR)...";

    const ocrResp = await fetch(`${GEMINI_ENDPOINT}?key=${apiKey}`, {
      method: "POST",
    ...
    if (!ocrResp.ok) {
      const errData = await ocrResp.json().catch(() => ({}));
      throw new Error(errData.error?.message || `HTTP ${ocrResp.status}`);
    }

    const ocrData = await ocrResp.json();
    const ocrRawText = ocrData.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!ocrRawText) throw new Error("Tidak ada respon OCR dari Gemini API.");

    const cleanOcr = ocrRawText.replace(/```json\n?/g, "").replace(/```\n?/g, "").trim();
    ...
    const productNameOcr = ocrParsed.productName || "Produk Kemasan";
    const originOcr = ocrParsed.countryOfOrigin || "";
...
Return STRICT JSON ONLY:
{
  ...
  "halalStatus": "HALAL",
  "overallVerdictBadge": "HALAL & AMAN",
  ...
  "storageTip": "Storage tip in Indonesian"
    ...
    const auditRawText = auditData.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!auditRawText) throw new Error("Tidak ada respon audit dari Gemini API.");

    const cleanAudit = auditRawText.replace(/```json\n?/g, "").replace(/```\n?/g, "").trim();
